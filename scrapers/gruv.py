...
class Gruv(Scraper):
    ...
    def __init__(self, category: str):
        ...
        self.session.headers['Referer'] = 'https://www.gruv.com/category/4k_ultra_hd_bluray_movies_pre_order' # fair enough, it's a form
    ...
    def scrape(self):
        ...
        for card in cards:
            ...
            image_tag = card.select_one('.card-img-top')
            
            if not price_tag or not title_tag or not image_tag:
                print('Invalid card, missing price, title, or image tag', card)
                continue
            price = float(price_tag.text.strip('$ '))
            title = title_tag.text
            url = self.base_url + title_tag.attrs.get('href', '/') # /product/...
            img = image_tag.attrs.get('src', '')
            found.append({
                'price': price,
                'title': title,
                'url': url,
